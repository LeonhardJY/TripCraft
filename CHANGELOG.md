...
### 全栈重构：用户 LeonhardJY 接手后全面改造

#### 架构迁移
- 向量数据库：ChromaDB → **Qdrant**（Docker 容器化）
- 大模型：阿里云 DashScope（付费 API）→ **Ollama 本地推理**（qwen2.5:3b + GPU 加速，完全免费）
- Embedding：qwen3.7-text-embedding → **nomic-embed-text**（本地 274MB 模型）
- Rerank：外部 DashScope API → 纯规则级重排序，零外部依赖

#### 前端全面重设计
- UI 框架：移除 Ant Design Vue，改为纯自定义 CSS 设计系统
- 设计语言：暖米白 #F4F1EA + 珊瑚橙 #D97757 + Instrument Serif 衬线字体
- 路由：v-if 切页 → **Vue Router**（/result /history）
- 状态管理：props/emit → **Pinia** 集中管理
- 页面：杂志式两栏首页、目的地风景墙、三步流程、工具箱、随机用户感言
- 动效：滚动渐入（v-reveal）、卡片 hover 微动效、暗色模式

#### 后端适配
- config.py：ChromaDB → Qdrant
- vector_db.py：完全重写，使用 Qdrant + Ollama 原生 `/api/embed`
- retriever.py：去掉外部 Rerank 依赖
- .env：统一指向本地服务

#### 项目清理
- 移除废弃前端旧视图
- requirements.txt：chromadb → qdrant-client
- Dockerfile：适配 Qdrant
